module.exports = {
    ...
    countyStats: data => {
        ...
        if (wales != null) {
            counties.push({ total: walTotal, visited: wales?.total ?? 0, name: "Wales" });
        }
        counties.push({ total: englandTotal + data.countiesNotOnMap.notOnMapCount + scotTotal + walTotal, visited: englandVisited + (scotland?.total ?? 0) + (wales?.total ?? 0), name: "Total" });
